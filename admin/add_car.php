...
$allimages = [];
$count = 0;
$imageError1 = $imageError2 = $imageError3 = $rcBookError = $insuranceError = "";
...
if (isset($_POST['submit'])) {
    ...
    // File validation for RC Book & Insurance
    ...
    if (!empty($_FILES['rcbook']['name']) && !validateFile($_FILES['rcbook'], $allowedTypes)) {
        $rcBookError = "Only JPG, PNG, and PDF files are allowed for RC Book.";
        $count++;
    }
    if (!empty($_FILES['insurance']['name']) && !validateFile($_FILES['insurance'], $allowedTypes)) {
        $insuranceError = "Only JPG, PNG, and PDF files are allowed for Insurance.";
        $count++;
    }
    ...
    if ($count == 0) {
        ...
        // Upload RC Book & Insurance
        $rcBookPath = "";
        $insurancePath = "";

        if (!empty($_FILES['rcbook']['name'])) {
            $rcBookPath = "uploads/" . $_FILES['rcbook']['name'];
            move_uploaded_file($_FILES['rcbook']['tmp_name'], $rcBookPath);
        }
        if (!empty($_FILES['insurance']['name'])) {
            $insurancePath = "uploads/" . $_FILES['insurance']['name'];
        ...
        if (count($allimages) > 0) {
            ...
            $insert = "insert into car_list (cname,modal,chprice,price,no_plate,brand,image,seat,fual,door,en_power,en_type,break_type,fual_capacity,mileage,accessories, rcbook, insurance)
    values ('$car_name',$modal,$chprice,$rent_price,'$no_plate','$brand','$imagePathsWithoutimg',$seat,'$fual',$door,'$power','$engine','$break',$f_tank,$mile,'$accessories_list', '$rcBookPath', '$insurancePath')";
            $run = mysqli_query($conn, $insert);
    ...
    <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="car_details">
                ...
                <div class="up-img">
                    Upload Rc Book :<span style="color: red;">*</span>
                    <input type="file" name="rcbook" multiple onchange="previewImages(this, 'imagePreview4')" accept="image/*,application/pdf"><br>
                    <div id="imagePreview4"></div> <!-- Preview container for image1 -->
                    <p style="color: red;"><?php echo $rcBookError; ?></p>

                </div>

                <div class="up-img">
                    Upload Insurance File :<span style="color: red;">*</span>
                    <input type="file" name="insurance" multiple onchange="previewImages(this, 'imagePreview5')" accept="image/*,application/pdf"><br>
                    <div id="imagePreview5"></div> <!-- Preview container for image1 -->
                    <p style="color: red;"><?php echo $insuranceError; ?></p>
